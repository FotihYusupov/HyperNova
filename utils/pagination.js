...
const paginate = async (model, query, ...populateFields) => {
  try {
    ...
    return {
      ...
      _links: {
        self: `http://localhost:3001/api/${model.modelName}/page=${page}&page=${perPage}`, // Self link
        first: `http://localhost:3001/api/${model.modelName}/page=1&page=${perPage}`, // First page link
        prev: page > 1 ? `http://localhost:3001/api/${model.modelName}/page=${+page - 1}&page=${perPage}` : null, // Previous page link
        next: page < totalPages ? `http://localhost:3001/api/${model.modelName}/page=${+page + 1}&page=${perPage}` : null, // Next page link
        last: `http://localhost:3001/api/${model.modelName}/page=${totalPages}&page=${perPage}`, // Last page link
      },
    };
  } catch (error) {
